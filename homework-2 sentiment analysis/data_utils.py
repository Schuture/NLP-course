...
class StanfordSentiment:
    def __init__(self, path=None, tablesize = 1000000):
        if not path:
            path = "D:/学习/课程/大数据/自然语言处理/16307110231_homework-2/datasets"
    ...
    def tokens(self):
        ...
        self._tokens = tokens       # a dict contains tokens and their index
        self._tokenfreq = tokenfreq # token frequency
        self._wordcount = wordcount # total word number
        self._revtokens = revtokens # all tokens that appear
        return self._tokens
    ...
    def sentences(self):
        ...
        self._sentences = sentences # a list contains all sentences (tokenized)
        self._sentlengths = np.array([len(s) for s in sentences]) # sentences length
        self._cumsentlen = np.cumsum(self._sentlengths) # the cumulative sum of length
